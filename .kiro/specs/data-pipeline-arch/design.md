...
enum class PacketType : uint8_t {
    ...
    component_info_update = 2     // Обновление информации о компоненте
};
...
struct registered_component_t {
    QTcpSocket *socket;              // TCP-соединение с компонентом
...
3. Отправляет сообщение регистрации с информацией о компоненте
   ...
   - Данные: registration_data_t (uuid, component_type, component_server_port)
...
6. Если таймаут или ошибка:
   ...
   - Логируется ошибка
   - Повторная попытка через CLIENT_RECONNECT_TIMEOUT_MS
```
...
3. Отправляет пакет component_info_update агрегатору
   - Данные: component_state_t (uuid, new_state)
4. Агрегатор обновляет информацию о компоненте
5. Агрегатор рассылает уведомление об изменении состояния всем подписчикам
```
...
- `disconnected` → `off`: через 60 секунд отсутствия соединения
...
- Переход в состояние `off`: 60 секунд отсутствия соединения
