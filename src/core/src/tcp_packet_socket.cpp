...
TcpBDSPSocket::TcpBDSPSocket(QObject *parent)
    ...
    _transceiver->get_write_stream().set_stream_writer([](uint8_t byte, void *ctx) {
        auto *socket = reinterpret_cast<TcpBDSPSocket *>(ctx);
    ...
    _transceiver->set_error_handler([](BDSP::parse_packet_status_t status, void *ctx) {
        auto tcp_bdsp_socket = reinterpret_cast<TcpBDSPSocket *>(ctx);
...
void TcpBDSPSocket::send_data(uint8_t packet_id, uint8_t *data, size_t size) {
