#include "analyzer/analyze/fft.h"
#include <QDebug>

#include "analyzer/dsp/fft/include/dsps_fft2r.h"
#include "analyzer/dsp/fft/include/dsps_fft4r.h"
...
FFT::fft_status_t FFT::run(std::vector<float> &samples, std::vector<float> &amplitudes) {
    ...
    size_t amplitudes_size = _samples_size / 2;
    if (samples.size() not_eq _samples_size or amplitudes.size() not_eq amplitudes_size) {
    ...
    if (_type == FFT2R) {
        ...
        if (ret not_eq ESP_OK) return FFT::INCORRECT_CONFIGS;
        ret = dsps_cplx2real_fc32_ansi_(_buffer.data(), _samples_size, dsps_fft4r_w_table_fc32,
